function [ ] = SEIA_extra(main_path,yr,rslt,mask,min_points,max_points,Dt,Rt)
% Main detection procedure of SEIA
% By Yikai Yang (email: yangyikai@scsio.ac.cn), 2022.7.29

str_sla=dir([main_path,'/SLA/SLA_*.mat']);
for year=1:length(str_sla)
    load([main_path,'/SLA/',str_sla(year).name]) % lon lat sla Time 
...
while ori<=length(ctr)
    points=ctr(2,ori+points); sla_val=ctr(1,ori); % sla contour value
...
ae_cir(length(x_max))=struct('time',[ ],'bound',[ ],'center',[ ],...
                              'radius',[ ],'amplitude',[ ],'eke',[ ],...
                              'vorticity',[ ],'McS',[ ],'alive',[ ]); % multi-core structure (McS)
                          
% search a biggest contour for each peak                          
occupy_num=[ ];
for i=1:length(sla_c)
    sla_val=sla_c(i).value;
    ...
    [~,lo]=find(in==1);
    if sum(in2)==0 && length(lo)==1 % бя Mononuclear
    ...
    elseif sum(in2)==0 && length(lo)>=2 % бя Polyonuclear: >=2
...
% extra calculation of amplitude, eke and vorticity
ae_cir=cal_amp_eke_vor(ae_cir,min_points,rslt);
...
if day==1 && year==1
    ...
    for n=1:length(ae_cir)
        ...
        [~,wh]=max(ratio);
        if ~isnan(ratio(wh)) && max(ratio)>=Rt % бя successfully tracked
    ...
    for i=missing_no % бя failed to be tracked
        ...
        if alive==1 % alive in the last time step
            ...
            ae(i).amplitude=[ae(i).amplitude;ae(i).amplitude(end)];
            ae(i).eke=[ae(i).eke;ae(i).eke(end)];
            ae(i).vorticity=[ae(i).vorticity;ae(i).vorticity(end)];            
    ...
    if ~isempty(ae_dead)
        eddy_output_extra(ae_dead,main_path,yr(year,:),'AE');
        clear ae_dead
    end
end
...
ce_cir(length(x_min))=struct('time',[ ],'bound',[ ],'center',[ ],...
                              'radius',[ ],'amplitude',[ ],'eke',[ ],...
                              'vorticity',[ ],'divergence',[ ],'McS',[ ],'alive',[ ]);
% search a biggest contour for each peak     
for i=1:length(sla_c)
    sla_val=sla_c(i).value;
    llon=sla_c(i).lon;llat=sla_c(i).lat;
    ...
    if sum(in2)==0 && length(lo)==1 
            ...
            ce_cir(lo).bound=[ce_cir(lo).bound,{[llon,llat]}];
            ce_cir(lo).center=[x_min(lo),y_min(lo)]; 
            % ce_cir(lo).center=[mean(llon),mean(llat)];
            ce_cir(lo).amplitude=[ce_cir(lo).amplitude;sla_val];
            ce_cir(lo).radius=[ce_cir(lo).radius;radius_compute(x_min(lo),y_min(lo),llon,llat)]; % Compute the radius
...
% extra calculation of amplitude, eke and vorticity
ce_cir=cal_amp_eke_vor(ce_cir,min_points,rslt);
...
if day==1 && year==1
    ...
    for n=1:length(ce_cir)
        ...
        if ~isnan(ratio(wh)) && max(ratio)>=Rt % бя successfully tracked
            ...
            ce(track).amplitude=[ce(track).amplitude;ce_cir(n).amplitude];
            ce(track).eke=[ce(track).eke;ce_cir(n).eke];
            ce(track).vorticity=[ce(track).vorticity;ce_cir(n).vorticity];            
            ...
            ce(new).amplitude=ce_cir(n).amplitude;
            ce(new).eke=ce_cir(n).eke;
            ce(new).vorticity=ce_cir(n).vorticity;            
    ...
    for i=missing_no % бя failed to be tracked
        ...
        if alive==1  % alive in the last time step
            ...
            ce(i).amplitude=[ce(i).amplitude;ce(i).amplitude(end)];
            ce(i).eke=[ce(i).eke;ce(i).eke(end)];
            ce(i).vorticity=[ce(i).vorticity;ce(i).vorticity(end)];            
    ...
    if ~isempty(ce_dead)
        eddy_output_extra(ce_dead,main_path,yr(year,:),'CE');
        clear ce_dead
    end
end
close all
